...
router.get('/',        authenticate, authorize(['patient']),        getHistory);

// DELETE /api/ai/history/:id   — patient deletes own record
router.delete('/:id',  authenticate, authorize(['patient']),        deleteHistory);

// GET  /api/ai/history/admin   — admin sees all records
router.get('/admin',   authenticate, authorize(['admin']),           getAllHistory);
