...
const PaymentDetailsModal = ({ isOpen, onClose, payment, isDark }) => {
  ...
  useEffect(() => {
    const fetchDetails = async () => {
      ...
      setIsLoading(true);
      try {
        const patData = await getPatientByIdForDoctor(payment.patientId);
        console.log("Fetched Patient Data for Doctor:", patData);
        const patient = await getPatientById(patData.userId); 
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
              ...
              <div className={`p-6 rounded-2xl border ${isDark ? 'border-slate-800 bg-slate-900/30' : 'border-slate-100 bg-slate-50'}`}>
                ...
                <div className="space-y-5">
                  <div>
                    <p className={`text-xs font-bold mb-1 ${isDark ? 'text-slate-500' : 'text-slate-400'}`}>Patient</p>
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <p className={`text-xs ${isDark ? 'text-slate-400' : 'text-slate-500'}`}>
                          {patData?.patientId || 'Patient ID not available'}
                        </p>                       
                      </div>
                    </div>
                  </div>
